...
This is the user guide for the project. It will contain all the information needed to use the CLI application.
...
7. Note that the data is saved in the hard disk **automatically** after any command that changes the data. There is no need to save manually.
   Please take note that **deleting** the `tasks.dat` file will result in the **loss of all data**.
   Please ensure that the data file named `tasks.dat` is in the **same directory** as the `adam.jar` file to load the saved data.
...
`event <description> /from <start date> /to <end date>` - Adds an event task with the given description and start and end dates.

#### Example of usage
`todo read book` - Adds a todo task with the description "read book".

`deadline return book /by 2024-03-30` - Adds a deadline task with the description "return book" and the date "2024-03-30".

`event project meeting /from 2024-03-07 100 /to 2024-03-07 230`
- Adds an event task with the description "project meeting" and the start date Mar 07 2024, 1:00pm and the end date Mar 07 2024, 2:30pm.
