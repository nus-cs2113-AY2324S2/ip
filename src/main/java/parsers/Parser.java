...
public class Parser {
    ...
    public static Command parseCommand(String userInput) throws KikuException {
        if (userInput.trim().isEmpty()) {
            throw new KikuInvalidTaskException("Input cannot be empty. \n" + HORIZONTAL);
        }
        ...
        case "deadline":
            ...
            if (deadlineParts.length < 2) {
                throw new KikuInvalidTaskException("Please specify the deadline with '/by'. \n" +
                        HORIZONTAL);
            }
        ...
        case "event":
            ...
            if (eventParts.length < 2 || !eventParts[1].contains(" /to ")) {
                throw new KikuInvalidTaskException("Please specify the event times " +
                        "with '/from' and '/to'. \n" + HORIZONTAL);
            }
        ...
        case "delete":
            try {
                ...
                if (index < 0) {
                    throw new KikuInvalidTaskException("Index must be greater than 0. \n" +
                            HORIZONTAL);
                }
                return handleIndexBasedCommand(commandType, index);
            } catch (NumberFormatException e) {
                throw new KikuInvalidTaskException("Invalid format, " +
                        "please specify task index correctly. \n" + HORIZONTAL);
            }
        case "bye":
            return new ExitCommand();
        default:
            throw new KikuInvalidTaskException("Please specify a todo, deadline, or event! " +
